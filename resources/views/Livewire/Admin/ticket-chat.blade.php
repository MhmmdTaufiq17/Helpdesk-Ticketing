...
>
    ...
    <div class="px-5 py-3.5 bg-white border-b border-slate-200 flex items-center justify-between shrink-0 z-10 shadow-sm">
        <div class="flex items-center gap-3">
            <div class="relative flex items-center justify-center w-10 h-10 rounded-full bg-indigo-100 text-indigo-600">
            ...
            <div>
                ...
                <p class="text-[11px] font-medium {{ $ticket->status !== 'closed' ? 'text-emerald-500' : 'text-slate-400' }}">
                    {{ $ticket->status !== 'closed' ? 'Online (Active)' : 'Closed' }}
                </p>
            </div>
        </div>
            ...
            <div class="flex flex-col {{ $msg['sender_type'] === 'admin' ? 'items-end' : 'items-start' }} animate-fade-in">
                <div class="flex items-end gap-2 max-w-[85%] {{ $msg['sender_type'] === 'admin' ? 'flex-row-reverse' : 'flex-row' }}">
                    ...
                    @if($msg['sender_type'] !== 'admin')
                        <div class="w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center text-[10px] font-bold bg-white border border-slate-200 text-slate-600 shadow-sm">
                    ...
                    <div class="relative group px-4 py-2.5 shadow-sm
                        ...
                        {{-- Sender Name (hanya untuk klien/user) --}}
                        @if($msg['sender_type'] !== 'admin')
                            <p class="text-[10px] font-bold text-indigo-500 mb-0.5">{{ $msg['sender_name'] }}</p>
                        @endif

                        <p class="text-[13px] leading-relaxed whitespace-pre-wrap break-words">{{ $msg['message'] }}</p>
                        
                        <div class="flex items-center justify-end gap-1 mt-1">
                            <span class="text-[9px] {{ $msg['sender_type'] === 'admin' ? 'text-indigo-100' : 'text-slate-400' }}">
    ...
    @if ($ticket->status !== 'closed')
        <div class="p-3 bg-white border-t border-slate-200 shrink-0">
            <form wire:submit.prevent="sendMessage" class="flex items-end gap-2 relative">
                ...
                <div class="flex-1 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-2 focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-50 transition-all shadow-inner">
                    <textarea
                        wire:model="message"
                ...
                <button type="submit"
                    ...
                    class="h-11 min-w-[44px] flex items-center justify-center rounded-xl font-bold text-white bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
