...
<form method="GET" action="{{ route('admin.tickets.index') }}" id="filterForm">
    ...
    <div class="bg-white border border-gray-200 rounded-xl px-4 py-3 mb-4 flex items-center gap-3 flex-wrap">
        {{-- Search --}}
        <div class="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-lg px-3 flex-1 min-w-48 focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-100 focus-within:bg-white transition-all">
...
<div class="flex items-center gap-2 flex-wrap mb-4">
    ...
    @if(request('q'))
        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-600 border border-indigo-100">
            "{{ request('q') }}"
    ...
    @if(request('priority'))
        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-600 border border-indigo-100">
            ...
            <a href="{{ request()->fullUrlWithoutQuery(['priority']) }}" class="opacity-60 hover:opacity-100 ml-0.5">✕</a>
        </span>
    @endif
    @if(request('category'))
        ...
        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-600 border border-indigo-100">
            ...
            <a href="{{ request()->fullUrlWithoutQuery(['category']) }}" class="opacity-60 hover:opacity-100 ml-0.5">✕</a>
        </span>
    @endif
</div>
@endif
...
<div class="flex items-center justify-between mb-3 text-sm text-gray-400">
    <span>Menampilkan <span class="font-semibold text-gray-700">{{ $tickets->firstItem() ?? 0 }}–{{ $tickets->lastItem() ?? 0 }}</span> dari <span class="font-semibold text-gray-700">{{ $tickets->total() }}</span> tiket</span>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-100">
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">Kode</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Judul / Pelapor</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">Kategori</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">Prioritas</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">Status</th>
                    <th class="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">Masuk</th>
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-4 py-3.5 whitespace-nowrap">
                        <span class="font-mono text-xs font-semibold text-indigo-500">{{ $ticket->ticket_code }}</span>
                    </td>
                    <td class="px-4 py-3.5 max-w-xs">
                        <div class="font-medium text-gray-800 truncate" title="{{ $ticket->title }}">{{ $ticket->title }}</div>
                        <div class="text-xs text-gray-400 truncate mt-0.5">{{ $ticket->client_name }} · {{ $ticket->client_email }}</div>
                    </td>
                    <td class="px-4 py-3.5 whitespace-nowrap">
                    ...
                    <td class="px-4 py-3.5 whitespace-nowrap">
                        @if($ticket->status === 'open')
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">Open</span>
                        @elseif($ticket->status === 'in_progress')
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-700">In Progress</span>
                        @else
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700">Closed</span>
                        @endif
                    </td>
    ...
    <div class="flex items-center justify-between px-5 py-3.5 border-t border-gray-100 flex-wrap gap-3">
        ...
        <div class="flex gap-1">
            @if($tickets->onFirstPage())
                <span class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-300 opacity-40 cursor-not-allowed">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                </span>
            @else
                <a href="{{ $tickets->previousPageUrl() }}" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 hover:bg-gray-100 transition-all">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                </a>
            @endif
            ...
            @foreach($tickets->getUrlRange(max(1,$tickets->currentPage()-2), min($tickets->lastPage(),$tickets->currentPage()+2)) as $page => $url)
                @if($page == $tickets->currentPage())
                    <span class="w-8 h-8 flex items-center justify-center rounded-lg text-sm font-semibold bg-indigo-500 text-white">{{ $page }}</span>
                @else
                    <a href="{{ $url }}" class="w-8 h-8 flex items-center justify-center rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-100 transition-all">{{ $page }}</a>
                @endif
            @endforeach

            @if($tickets->hasMorePages())
                <a href="{{ $tickets->nextPageUrl() }}" class="w-8 h-8 flex items-center justify-center rounded-lg text-gray-500 hover:bg-gray-100 transition-all">
