...
<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
    <div class="flex-1 min-w-0">
        ...
        <p class="text-[13px] text-slate-500 m-0">Selamat datang, {{ auth()->user()->name }}. Berikut ringkasan tiket hari ini.</p>
    </div>
    <div class="flex gap-2 shrink-0 w-full sm:w-auto">
        <a href="{{ route('admin.reports.export') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-white text-slate-700 border-[1.5px] border-slate-200 rounded-[9px] font-['Sora',sans-serif] text-[13px] font-semibold no-underline transition-all hover:bg-slate-50 hover:border-slate-300 hover:text-slate-900">
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div class="stat-card bg-white border border-slate-200 rounded-[14px] p-[18px_20px] flex flex-col gap-3 shadow-[0_1px_4px_rgba(0,0,0,.07)] transition-shadow hover:shadow-[0_4px_16px_rgba(0,0,0,.09)]">
    ...
    <div class="stat-card bg-white border border-slate-200 rounded-[14px] p-[18px_20px] flex flex-col gap-3 shadow-[0_1px_4px_rgba(0,0,0,.07)] transition-shadow hover:shadow-[0_4px_16px_rgba(0,0,0,.09)]">
        ...
        <div class="text-[28px] font-extrabold text-slate-900 tracking-[-1px] leading-none">{{ $stats['in_progress'] }}</div>
...
<div class="bg-white border border-slate-200 rounded-[14px] shadow-[0_1px_4px_rgba(0,0,0,.07)] overflow-hidden">
    <div class="flex items-center justify-between px-5 py-4 border-b border-slate-200">
        ...
        <a href="{{ route('admin.tickets.index') }}" class="inline-flex items-center justify-center gap-1.5 px-[11px] py-[5px] bg-white text-slate-700 border-[1.5px] border-slate-200 rounded-[9px] font-['Sora',sans-serif] text-[12px] font-semibold no-underline transition-all hover:bg-slate-50 hover:border-slate-300 hover:text-slate-900">Lihat Semua</a>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full border-collapse text-[13px]">
            <thead>
                <tr>
                    <th class="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-[0.6px] border-b border-slate-200 whitespace-nowrap">Kode Tiket</th>
                    <th class="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-[0.6px] border-b border-slate-200 whitespace-nowrap">Judul</th>
                    <th class="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-[0.6px] border-b border-slate-200 whitespace-nowrap">Kategori</th>
                    <th class="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-[0.6px] border-b border-slate-200 whitespace-nowrap">Prioritas</th>
                    <th class="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-[0.6px] border-b border-slate-200 whitespace-nowrap">Status</th>
                    <th class="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-[0.6px] border-b border-slate-200 whitespace-nowrap">Masuk</th>
                    <th class="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase tracking-[0.6px] border-b border-slate-200 whitespace-nowrap"></th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr class="transition-colors hover:bg-slate-50">
                    <td class="px-4 py-3.5 border-b border-slate-200 align-middle">
                        <span class="font-['DM_Mono',monospace] text-[12px] text-indigo-600">{{ $ticket->ticket_code }}</span>
                    </td>
                    <td class="px-4 py-3.5 border-b border-slate-200 align-middle max-w-[220px]">
                        <span class="block overflow-hidden text-ellipsis whitespace-nowrap text-slate-900 font-medium">{{ $ticket->title }}</span>
                        <span class="text-[11.5px] text-slate-500">{{ $ticket->client_name }}</span>
                    </td>
                    <td class="px-4 py-3.5 border-b border-slate-200 align-middle">
                        <span class="text-[12.5px] text-slate-700">{{ $ticket->category->category_name ?? '—'  }}</span>
                    </td>
                    <td class="px-4 py-3.5 border-b border-slate-200 align-middle">
                        @if($ticket->priority === 'high')
                            <span class="inline-flex items-center px-2.5 py-1 bg-red-100 text-red-600 rounded-full text-[11.5px] font-semibold">Tinggi</span>
                        @elseif($ticket->priority === 'medium')
                            <span class="inline-flex items-center px-2.5 py-1 bg-amber-100 text-amber-700 rounded-full text-[11.5px] font-semibold">Sedang</span>
                        @elseif($ticket->priority === 'low')
                            <span class="inline-flex items-center px-2.5 py-1 bg-green-100 text-green-700 rounded-full text-[11.5px] font-semibold">Rendah</span>
                        @else
                    ...
                    <td class="px-4 py-3.5 border-b border-slate-200 align-middle">
                        @if($ticket->status === 'open')
                            <span class="inline-flex items-center px-2.5 py-1 bg-blue-100 text-blue-600 rounded-full text-[11.5px] font-semibold">Open</span>
                        @elseif($ticket->status === 'in_progress')
                            <span class="inline-flex items-center px-2.5 py-1 bg-amber-100 text-amber-700 rounded-full text-[11.5px] font-semibold">In Progress</span>
                        @else
                            <span class="inline-flex items-center px-2.5 py-1 bg-green-100 text-green-700 rounded-full text-[11.5px] font-semibold">Closed</span>
                        @endif
                    </td>
                    <td class="px-4 py-3.5 border-b border-slate-200 align-middle text-[12px] text-slate-500 whitespace-nowrap">
                        {{ $ticket->created_at->diffForHumans() }}
                    </td>
                    <td class="px-4 py-3.5 border-b border-slate-200 align-middle text-right">
                        <a href="{{ route('admin.tickets.show', $ticket) }}" class="inline-flex items-center justify-center gap-1.5 px-[11px] py-[5px] bg-white text-slate-700 border-[1.5px] border-slate-200 rounded-[9px] font-['Sora',sans-serif] text-[12px] font-semibold no-underline transition-all hover:bg-slate-50 hover:border-slate-300 hover:text-slate-900">
                            Detail
                        </a>
                    </td>
                </tr>
